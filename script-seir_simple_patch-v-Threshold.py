...
def f_dataframe(my_g, my_airports_df, my_latlong_df):
    ...
    # Data Frame object ''
    countries_df = pd.DataFrame(data=0,index=countries, columns=countries, dtype=int)
...
#    S = np.ceil(Y0[0:leap:epi_classes])
#    E = np.ceil(Y0[1:leap+1:epi_classes])
#    I = np.ceil(Y0[2:leap+2:epi_classes])
#    R = np.ceil(Y0[3:leap+3:epi_classes])
...
#   df.loc[df.Week_conf_cases < 0, 'Week_conf_cases'] = \
#        df.loc[df.Week_conf_cases < 0, 'Week_conf_cases'].apply(neg2abs)
...
#    iw.timedelta(days=1)
    ...
    for x in df['COUNTRY'].drop_duplicates():
        if x != 0: # horrible hack here
            iso_week = np.asarray(df[df['COUNTRY'] == x][['YEAR','WEEK']],dtype='int32')
#            fecha = [Week(i[0], i[1]).sunday() for i in iso_week]
            fecha = [Week(i[0], i[1]) for i in iso_week]
#            fecha = [Week(i[0], i[1]).thisweek() for i in iso_week]
            # print count, ' -- ', x, fecha
            country.append(x)
...
# EPI_CLASSES, BETA, SIGMA, GAMMA)
def f_aggregated_model_solution(m_factor, H, Y0, t_start, t_end, t_inc,t_range, epi_classes, beta, sigma, gamma):
    """
    this function receives:
        - a proportional  rescaling constant for the matrix connection m_factor
        - a network connection modle H (networkx object)
